...
function sortedWordFreq(wordFreq) {
    ...
    fs.appendFile('wordsFreq.json', JSON.stringify(output, null, 2), err => {
        // Handle Error
...
function rankMultFreq(wordFreq){
    const arr = Object.entries(wordFreq).sort((a, b) => b[1] - a[1]);
    const output = arr.map((pair, index) => ({
    word: pair[0],
    frequency: pair[1],
    ...
    fs.appendFile('rankMultFreq.json', JSON.stringify(output, null, 2), err => {
    // Handle Error
    // Not working properly
    });
}
